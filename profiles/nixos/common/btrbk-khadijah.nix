...
let
	...
	#backupLocation = "/mnt2/btr_backup/mahirah"; #"/mnt/btr_pool"; # Need: mkdir -p /btrbk_backups/mahirah
	baseBackupLocation = "/mnt2/btr_backup";
	backupLocation = "${baseBackupLocation}/mahirah"; #"/mnt/btr_pool"; # Need: mkdir -p /btrbk_backups/mahirah
in {
	# XXX: Do mount subvolid=5 to /mnt/btr_pool
	#fileSystems.${backupLocation} = {
	fileSystems.${baseBackupLocation} = {
	#fileSystems."/mnt2/btr_backup" = {
	...
	environment.etc."btrbk.conf".text = ''
		...
		target send-receive		/mnt2/btr_backup/mahirah
	'';
